...
import contactImg from "../assets/img/plot1.gif";
import 'animate.css';
...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.code == 200) {
      ...
      setStatus({ succes: false, message: 'Error, intente de nuevo'});
    }
  };
...
//DENTRO DEL CONTAINER, correspondencia suspendida falta de tiempo
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    </Col>
                    {
                      status.message &&
                      <Col>
                        <p className={status.success === false ? "Error" : "Hecho"}>{status.message}</p>
                      </Col>
                    }
                  </Row>
                </form>
